...
const SidebarFilter = ({ closeSidebar }: { closeSidebar?: () => void }) => (
  <div className="sidebar-content">
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
      <h3 style={{ fontSize: '18px', fontWeight: 'bold', color: '#333', margin: 0 }}> <Icon icon="material-symbols:filter-alt-outline" style={{ verticalAlign: 'middle' }} /> Filter</h3>
      {closeSidebar && (
    ...
    <div style={{ marginBottom: '24px' }}>
      <h4 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '12px', color: '#555' }}>Kisaran Harga</h4>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
        <input type="number" placeholder="Min" style={{ width: '100%', padding: '6px', fontSize: '13px', border: '1px solid #d1d5db', borderRadius: '4px' }} />
        <span style={{ color: '#999' }}>-</span>
        <input type="number" placeholder="Max" style={{ width: '100%', padding: '6px', fontSize: '13px', border: '1px solid #d1d5db', borderRadius: '4px' }} />
      </div>
      <button style={{ width: '100%', backgroundColor: '#ee4d2d', color: 'white', border: 'none', padding: '6px', borderRadius: '4px', fontSize: '12px', cursor: 'pointer' }}>TERAPKAN</button>
    </div>

    {/* Popular Filter */}
    <div style={{ marginBottom: '24px' }}>
      <h4 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '12px', color: '#555' }}>Urutkan</h4>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
          <input type="radio" name="sort" defaultChecked /> Terlaris
        </label>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
          <input type="radio" name="sort" /> Terbaru
        </label>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
          <input type="radio" name="sort" /> Harga: Rendah ke Tinggi
        </label>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
          <input type="radio" name="sort" /> Harga: Tinggi ke Rendah
        </label>
      </div>
    </div>
  </div>
...
const Pagination = () => (
  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', marginTop: '40px' }}>
    <button style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #e5e7eb', background: 'white', borderRadius: '4px', cursor: 'pointer' }}>
      <Icon icon="mdi:chevron-left" />
    </button>
    <button style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #ee4d2d', background: '#ee4d2d', color: 'white', borderRadius: '4px', cursor: 'pointer' }}>1</button>
    <button style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #e5e7eb', background: 'white', borderRadius: '4px', cursor: 'pointer' }}>2</button>
    <button style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #e5e7eb', background: 'white', borderRadius: '4px', cursor: 'pointer' }}>3</button>
...
export default function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [isMobileFilterOpen, setIsMobileFilterOpen] = useState(false); // Retain for mobile filter UI
  const [searchQuery, setSearchQuery] = useState(''); // Retain for search input
  ...
  const filteredCategoryProducts = searchQuery
    ...
    <div style={{ backgroundColor: '#f5f5f5', minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <main className="main-content" style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', color: '#555' }}>
             <Link href="/paperlisens" style={{ textDecoration: 'none', color: '#ee4d2d' }}>Home</Link> 
             <span>/</span> 
             <span>Kategori</span> 
             <span>/</span> 
        ...
        <div className={`mobile-sidebar ${isMobileFilterOpen ? 'open' : ''}`}>
           <SidebarFilter closeSidebar={() => setIsMobileFilterOpen(false)} />
        </div>
